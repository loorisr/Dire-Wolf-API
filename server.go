// HTTP REST + WebSocket server that processes KISS frames, decodes AX.25/APRS,
// stores packets in a ring buffer, serves JSON endpoints, broadcasts to
// WebSocket clients, and optionally writes to a CSV log.
package main
...
// Packet is the enriched view of a received AX.25 frame stored and served by the API.
type Packet struct {
	ID        int64       `json:"id"`
...
// Stats holds aggregate counters exposed via GET /api/stats.
type Stats struct {
...
// NewServer creates a Server. If csvPath is non-empty, a CSV file is created
// and a header row is written immediately.
func NewServer(addr, tlsAddr, certFile, keyFile string, maxPackets int, csvPath string, kissClient *Client) *Server {
	s := &Server{
		addr:       addr,
		tlsAddr:    tlsAddr,
		certFile:   certFile,
	...
	if csvPath != "" {
		...
		if err != nil {
			log.Printf("[CSV] Failed to create %s: %v", csvPath, err)
		} else {
			s.csvFile = f
			s.csvWriter = csv.NewWriter(f)
			s.csvWriter.Write(csvHeader())
			s.csvWriter.Flush()
			log.Printf("[CSV] Writing to %s", csvPath)
		}
	}
...
// csvHeader returns the column names for the CSV output file.
func csvHeader() []string {
	return []string{
		"id", "timestamp", "port",
		"source", "dest", "via", "path", "pid",
		"info_raw", "raw_hex", "error",
		"aprs_type",
		"lat", "lon", "symbol", "speed_kmh", "course_deg", "altitude_m",
		"msg_addressee", "msg_text", "msg_id",
		"weather_temp", "weather_humidity", "weather_pressure",
		"weather_wind_dir", "weather_wind_speed", "weather_wind_gust",
		"weather_rain_hour", "weather_rain_day",
		"status", "comment",
	}
}

// writeCSV appends a row to the CSV file and flushes immediately.
// This is a no-op if csvPath was not configured.
func (s *Server) writeCSV(pkt *Packet) {
	if s.csvWriter == nil {
		return
	}
	s.csvMu.Lock()
	...
	row := []string{
		strconv.FormatInt(pkt.ID, 10),
		pkt.Timestamp.Format(time.RFC3339Nano),
		strconv.Itoa(int(pkt.Port)),
		pkt.Source,
		pkt.Dest,
		pkt.Via,
		pkt.Path,
		fmt.Sprintf("0x%02X", pkt.PID),
		pkt.InfoRaw,
		pkt.RawHex,
		pkt.Error,
	}
	...
	if pkt.APRS != nil {
		a := pkt.APRS
		row = append(row, string(a.Type))

		if a.Position != nil {
			row = append(row,
				strconv.FormatFloat(a.Position.Lat, 'f', 6, 64),
				strconv.FormatFloat(a.Position.Lon, 'f', 6, 64),
				a.Position.Symbol,
				optFloat(a.Position.Speed, 2),
				optInt(a.Position.Course),
				optFloat(a.Position.Altitude, 1),
			)
		} else {
			row = append(row, "", "", "", "", "", "")
		}
		...
		if a.Message != nil {
			row = append(row, a.Message.Addressee, a.Message.Text, a.Message.MsgID)
		} else {
			row = append(row, "", "", "")
		}
		...
		if a.Weather != nil {
			...
			row = append(row,
				optFloat(w.Temp, 1),
				optInt(w.Humidity),
				optFloat(w.Pressure, 1),
				optInt(w.WindDir),
				optFloat(w.WindSpeed, 2),
				optFloat(w.WindGust, 2),
				optFloat(w.RainHour, 2),
				optFloat(w.RainDay, 2),
			)
		} else {
			row = append(row, "", "", "", "", "", "", "", "")
		}

		row = append(row, a.Status, a.Comment)
	} else {
		row = append(row,
			"",                                                     // aprs_type
			"", "", "", "", "", "",                            // position
...
func optFloat(v *float64, prec int) string {
	if v == nil {
		return ""
	}
...
// Start begins processing KISS frames and listening for HTTP connections.
// It blocks until the context is cancelled or a fatal listen error occurs.
...
// processFrames reads from the KISS channel and passes each frame to ingest.
...
// ingest decodes a KISS frame into a Packet, stores it in the ring buffer,
// writes it to the CSV file (if enabled), and broadcasts to WebSocket clients.
func (s *Server) ingest(frame KISSFrame) {
	...
	ax25Frame, err := DecodeAX25(frame.Data)
	if err != nil {
		...
		if ax25Frame.Type == "UI" && ax25Frame.PID == 0xF0 {
			pkt.APRS = DecodeAPRS(ax25Frame.Info, pkt.Source, pkt.Dest)
		}
	}
...
// broadcast JSON-marshals a packet and sends it to every connected WebSocket
// client. Dead connections are removed from the client set.
...
// selfSignedTLS generates an in-memory self-signed ECDSA P-256 certificate
// valid for localhost/127.0.0.1 for one year.
...
// GET /api/packets?limit=N&offset=N — returns newest-first paginated list.
...
// GET /api/packets/{id} — returns a single packet by its auto-increment ID.
...
// GET /api/stats — aggregate packet counters and uptime.
...
// GET /api/status — KISS connection state.
...
// GET /ws — upgrades to WebSocket, replays last 50 packets, then broadcasts
// live packets. The read loop detects client disconnection.
...
// GET / — embedded Web UI with Leaflet map, live packet list, and detail inspector.
